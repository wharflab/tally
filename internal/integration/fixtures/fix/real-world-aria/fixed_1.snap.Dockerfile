...
ADD --link https://github.com/aria2/aria2.git?ref=release-1.37.0 /aria2
...
set -e
autoreconf -i
./configure --disable-bittorrent --disable-metalink --disable-ftp --without-libxml2 \
	--without-libexpat --without-gnutls --without-sqlite3 --without-libssh2 --disable-nls \
	--enable-static ARIA2_STATIC=yes --enable-websocket --enable-libaria2 --with-openssl \
	--with-libcares --with-ca-bundle=/etc/ssl/certs/ca-certificates.crt
make -j$(nproc)
make install
EOF
...
set -e
apt-get update
apt-get install -y --no-install-recommends sudo gnupg gpg-agent software-properties-common
add-apt-repository -y ppa:ondrej/nginx-mainline
apt-get update
apt-get install -y --no-install-recommends nginx libnginx-mod-http-lua tzdata ca-certificates curl \
	openssl unzip
...
rm /tmp/ariang.zip
sed -i '1i env ARIA2_SECRET;' /etc/nginx/nginx.conf
sed -i '1i env ARIA2_PORT;' /etc/nginx/nginx.conf
EOF
...
if curl --output /dev/null --silent --fail --retry 10 --retry-delay 1 --retry-connrefused \
...
exec sudo -u aria2 aria2c --conf-path=/aria2/aria2.conf --rpc-listen-port=\${ARIA2_PORT} \
	--rpc-secret="\${ARIA2_SECRET}" --dir="\${CONTENT_ROOT_DIR}/downloads" \
	...
	--header="Authorization: Bearer \${HF_TOKEN}" --console-log-level=\${ARIA_LOG_LEVEL}
EOF
