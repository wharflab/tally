...
# install some tools
RUN choco install git pypy3 python
RUN python -m pip install -U pip pipx pip-run
RUN setx path "$env:Path;C:\Users\ContainerAdministrator\.local\bin;C:\programdata\chocolatey\lib\pypy3\tools\pypy3.7-v7.3.4-win32;C:\programdata\chocolatey\lib\pypy3\tools\pypy3.7-v7.3.4-win32\Scripts"
