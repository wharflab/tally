...
func boolTrue() *bool  { return new(true) }
func boolFalse() *bool { return new(false) }
	...
	testutil.RunRuleTests(t, NewMaxLinesRule(), []testutil.RuleTestCase{
		{
			...
			Content:        "FROM alpine\nRUN echo hello\nRUN echo world",
			Config:         MaxLinesConfig{Max: new(0)},
			WantViolations: 0,
		},
		{
			Name:           "no violation when under limit",
			Content:        "FROM alpine",
			Config:         MaxLinesConfig{Max: new(10)},
			WantViolations: 0,
		},
		{
			Name:           "no violation when at limit",
			Content:        "FROM alpine\nRUN echo hello",
			Config:         MaxLinesConfig{Max: new(2)},
			WantViolations: 0,
		},
		{
		...
		},
		{
			...
			Content:        "FROM alpine\n\nRUN echo hello\n\n",
			Config:         MaxLinesConfig{Max: new(2), SkipBlankLines: boolTrue()},
			WantViolations: 0, // Only 2 non-blank lines
		},
		{
			Name:           "count blank lines when false",
			Content:        "FROM alpine\n\nRUN echo hello",
			Config:         MaxLinesConfig{Max: new(2), SkipBlankLines: boolFalse()},
			WantViolations: 1, // 3 lines including blank
		},
		{
			Name:           "skip comments",
			Content:        "# This is a comment\nFROM alpine\n# Another comment",
			Config:         MaxLinesConfig{Max: new(1), SkipComments: boolTrue()},
			WantViolations: 0, // Only 1 non-comment line
		},
		{
			Name:           "count comments when false",
			Content:        "# Comment\nFROM alpine",
			Config:         MaxLinesConfig{Max: new(1), SkipComments: boolFalse()},
...
func TestMaxLinesRule_ValidateConfig(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "valid config",
			config:  MaxLinesConfig{Max: new(100)},
			wantErr: false,
		},
		{
		...
		},
		{
			...
			config:  MaxLinesConfig{Max: new(-1)},
			wantErr: true,
		},
		{
		...
		},
		{
			...
			config:  &MaxLinesConfig{Max: new(50)},
			wantErr: false,
		},
		{
		...
		},
		{
			...
			config:  &MaxLinesConfig{Max: new(-5)},
			wantErr: true,
		},
		{
