...
func TestBuilderRealWorldTeamCityNanoServerFixtureInfersWindowsSecondStage(t *testing.T) {
	...
	content, err := os.ReadFile(filepath.Join("..", "integration", "fixtures", "fix", "real-world-fix-teamcity-nanoserver", "Dockerfile"))
	if err != nil {
...
func TestBuilderRealWorldPowerShellAlpineFixtureStaysLinux(t *testing.T) {
	...
	content, err := os.ReadFile(filepath.Join("..", "integration", "fixtures", "fix", "real-world-fix-powershell-alpine", "Dockerfile"))
	if err != nil {
		t.Fatalf("read fixture: %v", err)
	}
