...
- **Sync invocation rewrites.** Each offending invocation is rewritten in place. When the source command is a simple HTTP `GET` that follows
  redirects, fails on HTTP status, and has a literal URL and plain sink (explicit `-o`/`-O` file, stdout pipe, or remote-name), tally lowers it
  locally through a shared `HTTPTransferOperation` IR and emits a concrete replacement edit. Commands outside that subset fall back to an async
  AI AutoFix `SuggestedFix` — applying that fallback additionally requires an ACP-capable agent configured in the top-level `[ai]` section.
- **Post-sync cleanup.** After every sync fix has been applied (including edits from peer rules like `tally/sort-packages` and
  `tally/prefer-curl-config`), DL4001's async cleanup resolver re-parses the modified Dockerfile and removes anything that's now stale for the
  evicted tool:
  - the tool's entry in install commands (package-only for multi-package installs, the whole install-only RUN for single-package installs),
  - any `.curlrc`/`.wgetrc`-style COPY heredocs that point at the tool's config file,
  - any `CURL_HOME` / `WGETRC` / `WGETHOSTS` ENV bindings,
  - tally-authored annotation comments that precede those instructions.

Running cleanup after peer-rule sync fixes is what lets DL4001 compose with rules that touch the install line (`sort-packages`) or add tool
config (`prefer-curl-config`) without conflicting edit ranges.
